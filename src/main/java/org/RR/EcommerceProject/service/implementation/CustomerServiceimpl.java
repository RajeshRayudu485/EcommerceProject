...
import org.RR.EcommerceProject.repository.customerOrderRepository;
import org.RR.EcommerceProject.repository.itemRepository;
...
public class CustomerServiceimpl implements CustomerService {
	...
	public String addToCartItem(HttpSession session, int id) {
		if (session.getAttribute("customer") != null) {
			...
			Product product = (Product) productRepository.findByName(item.getName()).get(0);
	...
	public String removeFromCartItem(HttpSession session, int id) {
		if (session.getAttribute("customer") != null) {
			Item item = itemRepository.findById(id).orElseThrow();
			Product product = (Product) productRepository.findByName(item.getName()).get(0);
			return removeFromCart(product.getId(), session);
		} else {
			session.setAttribute("failure", "Invalid Session, Login Again");
			return "redirect:/login";
		}
